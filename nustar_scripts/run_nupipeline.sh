#!/bin/sh
# usage: ./run_nupipeline.sh PATH/TO/OBSID

# syntax: ./run_nupipeline.sh INDIR
if [ $# != 1 ] ; then
    echo Syntax:  ./run_nupipeline.sh PATH_TO_OBSID
    exit 1
fi

# Note, PATH_TO_OBSID is assumed to be relative to the current
# directory. It should be the parent directory that contains the NuSTAR
# data. For example:
...
# From right:
# The configuration below is for "high rate" events:
# A "0" means "require this bit to be zero" while an "x" means
# ignore this bit.
#
# 0th bit: bad pixels from CALDB bad pixel file. Leave this on.
...
# In this version we don't use the "high rate" type command. 
# See run_nupipeline_highrate.sh for an example there.
